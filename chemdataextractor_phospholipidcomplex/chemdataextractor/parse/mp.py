...
solvent_phrase = (R('^(re)?crystalli[sz](ation|ed)$', re.I) + (I('with') | I('from')) + cem | solvent_name)('mp_phrase')
cem_mp_phrase = (Optional(solvent_phrase) + Optional(cem) + Optional(I('having')).hide() + Optional(delim).hide() + Optional(bracket_any).hide() + Optional(delim).hide() + Optional(lbrct) + mp + Optional(rbrct))('mp_phrase')
...
class MpParser(BaseParser):
    ...
    def interpret(self, result, start, end):
        ...
        melting_point = self.model(value=first(result.xpath('./mp/value/text()')),
                                   ...
                                   solvent = first(result.xpath('./mp_phrase/solvent_phrase/text()')))
